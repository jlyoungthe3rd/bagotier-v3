...
| `src/features/character/`         | SVG paper doll figure, spatial equipment slots, and stat computation panel                       | `src/features/character/CharacterView.tsx`                               |
| `src/features/github/`            | Header repository link with GitHub SVG icon                                                      | `src/features/github/GitHubLink.tsx`                                     |
| `src/features/inventory/`         | Bag grid, inventory items, keyboard navigation, and floating tooltip integration                 | `src/features/inventory/InventoryGrid.tsx`                               |
...
| --------------------------------- | --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| `src/store/`                      | Session/UI state (equipped item IDs, bag slots, audio mute, focus tracking, pure transitions) | Must never hold full `Item` entity objects (invariant I5); no direct DOM or audio side effects in transitions |
| `src/features/*/use*Query.ts`     | TanStack Query fetch hooks, cache selectors (`useItem`), and server-state lifecycle           | Must not manage transient UI session state (bag order, mute toggle, focus)                                    |
...
- File naming pattern:
  - React components: PascalCase with `.tsx` extension (`CharacterView.tsx`, `InventoryGrid.tsx`, `EquipmentSlot.tsx`, `StatPanel.tsx`).
